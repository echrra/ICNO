...
parser.add_argument('--save_name', type=str, default='ICNO_era5_p')
...
data_path = args.data_path + '/ERA5/ERA5_P.grib'
...
def main():
    ...
    if eval:
        ...
        with torch.no_grad():
            ...
            def asymmetric_gamma_norm(vmin, vmax, mid, gamma_low=0.6, gamma_high=2.0):
                
                mid_norm = (mid - vmin) / (vmax - vmin)
                
                def forward(x):
                    x = np.clip((x - vmin) / (vmax - vmin), 0, 1)
                    y = np.where(
                        x < mid_norm,
                        0.5 * np.power(x / mid_norm, gamma_low),
                        0.5 + 0.5 * np.power((x - mid_norm) / (1 - mid_norm), 1 / gamma_high),
                    )
                    return y
                ...
                def inverse(y):
                    ...
                    x = np.where(
                        y < 0.5,
                        mid_norm * np.power(y / 0.5, 1 / gamma_low),
                        mid_norm + (1 - mid_norm) * np.power((y - 0.5) / 0.5, gamma_high),
                    )
            ...
            vmin, vmax = 97000, 103000
            mid = 101000
            norm = asymmetric_gamma_norm(vmin, vmax, mid, gamma_low=1.0, gamma_high=3.0)
            ...
            for col, (idx, year) in enumerate(zip(indices, years), 1):
                ...
                plt.imshow(true[idx].numpy(), cmap='gist_ncar', norm=norm, extent=[0, 360, -90, 90], origin='lower',interpolation='Gaussian')
                ...
                plt.imshow(pred[idx].numpy(), cmap='gist_ncar', norm=norm, extent=[0, 360, -90, 90], origin='lower',interpolation='Gaussian')
                ...
                plt.imshow((pred[idx] - true[idx]).numpy(), cmap='gist_ncar', extent=[0, 360, -90, 90], origin='lower',interpolation='Gaussian')
            ...
            save_fig = f'./results/{save_name}/summary_p.png'
